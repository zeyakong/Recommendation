...
def linear_regression():
    # this function uses TensorFlow to train the model
    # it will find the param a for real = a * rating-based + (1-a) * text-based

    # this function will also load the training dataset
    # the training dataset contains two list
    # one is : [rating-based prediction , text-based prediction]
    # two is : [real score]
    ...
    for one in test_set:
        ...
        text_score = wm_predict(rid=rid, uid=uid, rm=rm, wm=bag_200, maps=maps)
        rating_score = rm_predict(rid=rid, uid=uid, rm=rm, alg='cosine')
        y = Weights * rating_score + (1 - Weights) * text_score

        loss = tf.reduce_mean(tf.square(y - real_score))
