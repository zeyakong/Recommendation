...
def get_similarity_list(matrix, index):
    list1 = []
    list2 = []
    list3 = []
    vector1 = np.array(matrix[index])
    print([j for j, e in enumerate(np.array(matrix)[index]) if e != 0])

    # lengh = len(matrix)
    # for i in [j for j, e in enumerate(np.array(matrix)[index]) if e != 0]:
    #     if i != index:
    #         vector2 = np.array(matrix[i])
...
def test(size=200):
    ...
    for i in range(0, size):
        ...
        # print(real_score)
        # change the matrix. Hide the real item value.
        matrix[rid][uid] = 0
        o_score, cos_score, pearson_score = predict(rid, uid, 'all')
        # retrieve the matrix
        matrix[rid][uid] = real_score
    ...
    # print("Finished. RMSE:", RMSE(records))
    # print("Finished. MAE:", MAE(records))
...
def get_score(restaurant_index, user_index, sim_user_index_list, sim_rest_index_list):
    # find the average score for 200 similar users chose this restaurant.
    count = 0
    number = 0
    ...
    if count == 0:
        ...
        score1 = number / count
    # find the average score for the 200 similar restaurants.
    number = 0
    count = 0
